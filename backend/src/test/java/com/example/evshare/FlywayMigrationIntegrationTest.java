...
class FlywayMigrationIntegrationTest {
    ...
    @Test
    @DisplayName("Flyway schema version should be at version 13 with all migrations applied")
    void testFlywayMigrationsApplied() {
        MigrationInfo current = flyway.info().current();
        assertNotNull(current, "Current Flyway migration info must not be null");
        assertEquals("13", current.getVersion().getVersion(), "Current database version should be 13");
        assertTrue(current.getState().isApplied(), "Current migration must be in APPLIED state");

        MigrationInfo[] applied = flyway.info().applied();
        assertEquals(13, applied.length, "Exactly 13 migration scripts should be applied");
